📦
94435 /scripts/ui_reconstruct.js
✄
  ...
  async dump() {
    ...
    return new Promise((resolve, reject) => {
      frida_objc_bridge_default.schedule(frida_objc_bridge_default.mainQueue, function() {
        try {
          var keyWindow = frida_objc_bridge_default.classes.UIWindow.keyWindow();
          if (!keyWindow) {
            var windows = frida_objc_bridge_default.classes.UIWindow.allWindows();
...
function dumpView(view) {
  ...
  if (view.isKindOfClass_(frida_objc_bridge_default.classes.UILabel)) {
    result.type = "Label";
    var text = view.text();
